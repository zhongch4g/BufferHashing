...
#include "CCEH_buflog_limit_wal.h"
#include "cceh_util.h"
#include "histogram.h"
...
using namespace cceh_buflog_limit_wal;
...
public:
    ...
    Benchmark ()
        ...
        printf ("Single CCEH \n");
        createLog ();
        D_RW (hashtable_)->initCCEH (pop_, initialSize, FLAGS_bufferNum, FLAGS_thread, log_root->log_addrs);
    }
    ...
    Benchmark (uint32_t ins_num)
        ...
        D_RW (hashtable0_)->initCCEH (pop0_, initialSize, FLAGS_bufferNum0, FLAGS_thread, log_root->log_addrs);
        ...
        D_RW (hashtable1_)->initCCEH (pop1_, initialSize, FLAGS_bufferNum1, FLAGS_thread, log_root->log_addrs);
    }
    ...
    Benchmark (uint32_t ins_num, uint32_t ins_n)
        ...
        D_RW (hashtable2_)->initCCEH (pop2_, initialSize, FLAGS_bufferNum2, FLAGS_thread, log_root->log_addrs);
    }
    ...
    void ReadWhileWriting (ThreadState* thread) {
        ...
        if (thread->tid > FLAGS_writeThreads - 1) {
            ...
            while (key_iterator.Valid ()) {
                ...
                for (; j < batch && key_iterator.Valid (); j++) {
                    size_t ikey = key_iterator.Next ();
                    D_RW (hashtable_)->InsertWithLog (pop_, ikey, reinterpret_cast<Value_t> (ikey), thread->tid);
                }
                ...
                if (thread->tid < nthread) {
                    ...
                    if (thread->ycsb_gen.NextG (FLAGS_gChoice) == kYCSB_Write) {
                        mp0 = D_RW (hashtable0_)
                                  ->InsertWithLog (pop0_, ikey, reinterpret_cast<Value_t> (ikey), thread->tid);
                    } else {
                    ...
                    bool mp1 =
                        D_RW (hashtable1_)->InsertWithLog (pop1_, ikey, reinterpret_cast<Value_t> (ikey), thread->tid);
                    if (mp1) {
                        thread->privateStates.fetchMinorCompactions (1);
                    }
                } else {
        ...
        while (!duration.Done (batch) && key_iterator.Valid ()) {
            ...
            for (; j < batch && key_iterator.Valid (); j++) {
                ...
                if (thread->tid / nthread == 0) {
                    ...
                    D_RW (hashtable2_)->InsertWithLog (pop2_, ikey, reinterpret_cast<Value_t> (ikey), thread->tid);
                } else {
                    perror ("threads distribute error . \n");
                    exit (1);
                }
        ...
        while (key_iterator.Valid ()) {
            ...
            for (; j < batch && key_iterator.Valid (); j++) {
                ...
                if (thread->ycsb_gen.NextA () == kYCSB_Write) {
                    D_RW (hashtable_)->InsertWithLog (pop_, key, reinterpret_cast<Value_t> (key), thread->tid);
                    insert++;
                } else {
    ...
    void YCSBC (ThreadState* thread) {
        ...
        while (key_iterator.Valid ()) {
            ...
            for (; j < batch && key_iterator.Valid (); j++) {
                ...
                if (ret == reinterpret_cast<Value_t> (key), thread->tid) {
                    find++;
                }
            }
            thread->stats.FinishedBatchOp (j);
            ...
            for (; j < batch && key_iterator.Valid (); j++) {
                ...
                if (npolicy == 0) {
                    D_RW (vhashtable_[npolicy])
                        ->InsertWithLog (vpop_[npolicy], key, reinterpret_cast<Value_t> (key), thread->tid);
                } else if (npolicy == 1) {
                    D_RW (vhashtable_[npolicy])
                        ->InsertWithLog (vpop_[npolicy], key, reinterpret_cast<Value_t> (key), thread->tid);
                } else {
    ...
    void bufferexchange (ThreadState* thread) {
        ...
        while (!duration.Done (batch) && key_iterator.Valid ()) {
            ...
            for (; j < batch && key_iterator.Valid (); j++) {
                ...
                uint32_t npolicy = thread->tid / nthread;
                if (npolicy == 0) {
                    D_RW (hashtable0_)->InsertWithLog (pop0_, key, reinterpret_cast<Value_t> (key), thread->tid);
                } else if (npolicy == 1) {
                    D_RW (hashtable1_)->InsertWithLog (pop1_, key, reinterpret_cast<Value_t> (key), thread->tid);
                } else {
                    perror ("threads distribute error . \n");
                    exit (1);
                }
