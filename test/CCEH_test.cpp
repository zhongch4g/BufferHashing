...
int main (int argc, char* argv[]) {
    ...
    if (!ifs) {
        ...
        for (int i = 0; i < numData; i++) ifs >> keys[i];
        ifs.close ();
    ...
    if (!exists) {
        {  // INSERT
            ...
            cout << elapsed / 1000 << "\tusec\t"
                 << (uint64_t)(1000000 * (numData / (elapsed / 1000.0))) << "\tOps/sec\tInsertion"
                 << endl;
        }
    }
    ...
    {  // SEARCH
        ...
        clock_gettime (CLOCK_MONOTONIC, &end);
        elapsed = (end.tv_sec - start.tv_sec) * 1000000000 + (end.tv_nsec - start.tv_nsec);
        cout << elapsed / 1000 << "\tusec\t" << (uint64_t)(1000000 * (numData / (elapsed / 1000.0)))
             << "\tOps/sec\tSearch" << endl;
            ...
            for (int i = 0; i < numThreads; i++) {
                if (i != numThreads - 1)
                    insertingThreads.emplace_back (
                        thread (insert, chunk_size * i, chunk_size * (i + 1)));
                else
        ...
        for (int i = 0; i < numThreads; i++) {
            if (i != numThreads - 1)
                searchingThreads.emplace_back (
                    thread (search, chunk_size * i, chunk_size * (i + 1), i));
            else
