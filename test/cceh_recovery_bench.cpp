...
public:
    ...
    size_t trace_size_;
    pmem::obj::pool<struct CCEH> pop_;
    pmem::obj::persistent_ptr<CCEH> hashtable_;
    uint32_t ins_num_;
    Benchmark ()
        : num_ (FLAGS_num),
        ...
        pop_ = pmem::obj::pool<struct CCEH>::create (FLAGS_filepath.c_str (), "CCEH", POOL_SIZE,
                                                     CREATE_MODE_RW);
        if (!pop_.handle ()) {
        ...
        hashtable_ = pop_.root ();
        printf ("CCEH-BUFLOG \n");
        hashtable_->initCCEH (pop_, initialSize, FLAGS_thread);
    }

    Benchmark (bool recovery)
        : num_ (FLAGS_num),
          value_size_ (FLAGS_value_size),
          reads_ (FLAGS_read),
          writes_ (FLAGS_write),
          ...
          hashtable_ (OID_NULL) {
        remove (FLAGS_filepath.c_str ());  // delete the mapped file.
        pop_ = pmem::obj::pool<struct CCEH>::open (FLAGS_filepath.c_str (), "CCEH");
        if (!pop_.handle ()) {
            perror ("pmemoj_create");
            exit (1);
        }
    ...
    void ReadWhileWriting (ThreadState* thread) {
        ...
        if (thread->tid > 0) {
            ...
            while (key_iterator.Valid ()) {
                ...
                for (; j < batch && key_iterator.Valid (); j++) {
                    size_t ikey = key_iterator.Next ();
                    hashtable_->InsertWithLog (pop_, ikey, reinterpret_cast<Value_t> (ikey),
                                               thread->tid);
                }
    ...
    void DoWriteLat (ThreadState* thread) {
        ...
        while (key_iterator.Valid ()) {
            ...
            auto time_start = NowNanos ();
            hashtable_->InsertWithLog (pop_, ikey, reinterpret_cast<Value_t> (ikey), thread->tid);
            auto time_duration = NowNanos () - time_start;
            thread->stats.hist_.Add (time_duration);
        }
        return;
    ...
    void DoLoadFactor (ThreadState* thread) {
        ...
        while (key_iterator.Valid ()) {
            ...
            thread->stats.FinishedBatchOp (j);
            printf ("Load factor: %.3f\n", (double)(InsertWithLoged) / hashtable_->Capacity ());
        }
        return;
    }
    ...
    void YCSBA (ThreadState* thread) {
        ...
        while (key_iterator.Valid ()) {
            ...
            for (; j < batch && key_iterator.Valid (); j++) {
                ...
                if (thread->ycsb_gen.NextA () == kYCSB_Write) {
                    hashtable_->InsertWithLog (pop_, key, reinterpret_cast<Value_t> (key),
                                               thread->tid);
                    InsertWithLog++;
                } else {
                    auto ret = hashtable_->Get (key);
                    find++;
                }
            }
            thread->stats.FinishedBatchOp (j);
        ...
        char buf[100];
        snprintf (buf, sizeof (buf), "(InsertWithLog: %lu, read: %lu)", InsertWithLog, find);
        thread->stats.AddMessage (buf);
        return;
    }
    ...
    void YCSBF (ThreadState* thread) {
        ...
        char buf[100];
        snprintf (buf, sizeof (buf), "(read_modify: %lu, read: %lu)", InsertWithLog, find);
        thread->stats.AddMessage (buf);
        return;
    }
