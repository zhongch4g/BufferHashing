...
int ShuffleSeq (int i) { return sequence_shuffle.Next () % i; }
...
// uniform
TraceUniform::TraceUniform (int seed, uint64_t minimum, uint64_t maximum) : Trace (seed) {
    gi_ = new GenInfo ();
...
// exponential reverse
TraceExponentialReverse::TraceExponentialReverse (int seed, const double percentile, double offset,
                                                  double range)
    : Trace (seed), range_ (range) {
    range = range * 0.15;
    gi_ = new GenInfo ();
...
// ===================================================
TraceNormal::TraceNormal (int seed, uint64_t minimum, uint64_t maximum) : Trace (seed) {
    gi_ = new GenInfo ();
...
std::vector<YCSB_Op> YCSB_LoadGenerate (int64_t range, uint64_t max_num, YCSBLoadType type,
                                        Trace* trace,
                                        const std::vector<uint64_t>& ycsb_insertion_sequence) {
