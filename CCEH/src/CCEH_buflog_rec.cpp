...
void CCEH::BufferRecovery (pmem::obj::pool<CCEH> pop, int tid, vector<uint64_t> segidx) {
    // ensure the consistent of the directory entries
    DEBUG ("Start Recovery Buffer ..");
    // size_t i = 0;
    size_t cntRecovery = 0;
    ...
    uint64_t j = segiter.fetch_add (1, std::memory_order_relaxed);
    printf ("dirptr->capacity = %lu, segidx = %lu\n", dirptr->capacity, segidx.size ());

    while (segidx[j] < dirptr->capacity) {
        j = segiter.fetch_add (1, std::memory_order_relaxed);

        if (j >= dirptr->capacity) {
            // printf ("Finished Recovery (total = %lu) \n", cntRecovery);
            break;
        }

        auto ptr = D_RW (D_RW (D_RW (dir)->segment)[segidx[j]]);
        uint64_t logid = ptr->logPtr->getLogId ();
        uint64_t offset = ptr->logPtr->getOffset ();
