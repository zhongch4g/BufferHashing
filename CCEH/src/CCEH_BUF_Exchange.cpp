...
int Segment::initSegment (CCEH* _cceh) {
    bufferDestroy = false;
    nWriteBuffers = 0;
    ...
    //                          cceh->curSegnumNum.load (std::memory_order_relaxed);
    //     } else {
    ...
    //     }
    // }
    requiredBufNum = _cceh->bufferConfig.getKBufNumMax ();
    int32_t tmp = _cceh->bufnum.load (std::memory_order_relaxed);
    if (tmp < requiredBufNum) {
        int32_t BN = _cceh->bufnum.fetch_add (1, std::memory_order_relaxed);
        if (BN < requiredBufNum) {
...
int Segment::initSegment (size_t depth, CCEH* _cceh) {
    bufferDestroy = false;
    nWriteBuffers = 0;
    for (int i = 0; i < kNumSlot; ++i) {
        bucket[i].key = INVALID;
...
// void CCEH::releaseBuffer (PMEMobjpool* pop) {
// retry:
//     // 1. find out a segment to release its buffer
//     uint32_t randN = rand () % bufferConfig.bufferContainFlag->size ();
//     std::set<uint64_t>::iterator itlow;
//     itlow = bufferConfig.bufferContainFlag->lower_bound (randN);
...
//             goto retry;
//         }
//         bufferConfig.bufferContainFlag->erase (*itlow);
//         mergeBufAndSplitWhenNeeded (pop, target_ptr->bufnode_, target, 0);
//         target_ptr->bufnode_->Unlock ();
//         // 3. delete the buffer inside
//         if (target_ptr->bufnode_) {
//             delete target_ptr->bufnode_;
//             target_ptr->bufnode_ = NULL;
//         }
//         target_ptr->buf_flag = false;
//         // TODO : add 1 free buffer to global variable -> subtract 1 buffer from global variable
//         bufferConfig.fetchSubKBufNumMax (1);
//     }
// }

bool CCEH::releaseBuffer (PMEMobjpool* pop) {
retry:
    // 1. find out a segment to release its buffer
    uint32_t randN = rand () % bufferConfig.bufferContainFlag->size ();
    std::set<uint64_t>::iterator itlow;
    ...
    if (target_ptr->buf_flag) {
        ...
        if (D_RW (target)->bufnode_) {
            delete D_RW (target)->bufnode_;
            D_RW (target)->bufnode_ = NULL;
        }
        target_ptr->buf_flag = false;
        // TODO : add 1 free buffer to global variable -> subtract 1 buffer from global variable
        bufferConfig.fetchSubKBufNumMax (1);
...
void CCEH::initCCEH (PMEMobjpool* pop, size_t initCap, uint32_t bufferSizeFactor,
    ...
    bufferConfig.bufferContainFlag = new std::set<uint64_t> ();
    bufferConfig._bufferContainFlag = new std::set<uint64_t> ();
    bufferConfig.bufferNotContainFlag = new std::set<uint64_t> ();
    bufferConfig._bufferNotContainFlag = new std::set<uint64_t> ();
    ...
    printf ("Exchange -> initCCEH : %d, %d, %2.2f \n", bufferConfig.getKBufNumMax (),
    ...
    for (int i = 0; i < D_RO (dir)->capacity; ++i) {
        POBJ_ALLOC (pop, &D_RO (D_RO (dir)->segment)[i], struct Segment, sizeof (struct Segment),
                    NULL, NULL);
        int flag = D_RW (D_RW (D_RW (dir)->segment)[i])
                       ->initSegment (static_cast<size_t> (log2 (initCap)), this);
        if (flag == 1) {
            bufferConfig.bufferContainFlag->insert (i);
        } else {
            bufferConfig.bufferNotContainFlag->insert (i);
        }
        curSegnumNum.fetch_add (1, std::memory_order_relaxed);
    }
...
void CCEH::checkBufferData () {
    ...
    // printf ("%lu, %lu\n", segmentcnt, bufcnt);
    printf ("total = %d, bufferContainFlag->%d, bufferNotContainFlag->%d \n",
            bufferConfig.bufferContainFlag->size () + bufferConfig.bufferNotContainFlag->size (),
            bufferConfig.bufferContainFlag->size (), bufferConfig.bufferNotContainFlag->size ());
}
...
retry:
    ...
    if (!target_ptr->buf_flag) {
        /*
        // give buffer if there is extra buffer left
        uint32_t curBufNum = bufnum.load (std::memory_order_relaxed);
        if (curBufNum < bufferConfig.getKBufNumMax ()) {
            uint32_t fetafter = bufnum.fetch_add (1, std::memory_order_relaxed);
            if (fetafter < bufferConfig.getKBufNumMax ()) {
                target_ptr->bufnode_ = new WriteBuffer (target_ptr->local_depth);
                target_ptr->buf_flag = true;
                bufferConfig.bufferContainFlag->insert (x);
                goto retry;
            }
        }
        */
     ...
     * Access the target segment when this segment is spliting by other thread will cause the
     * difference of the depth.
     */
    if (D_RO (target)->local_depth != target_ptr->bufnode_->local_depth) {
        /* release the lock */
        target_ptr->bufnode_->Unlock ();
    ...
    if (isValidPut) {
        ...
        bufferWrites += 1;
        target_ptr->nWriteBuffers += 1;
        bufferMergeFlag = true;
        ...
        /*
            // to record the time interval that the segmenet buffer refill
            target_ptr->endt_ = target_ptr->timeval.NowMicros();
            target_ptr->interval_ = target_ptr->timeval.NowMicros () - target_ptr->startt_;
            target_ptr->startt_ = target_ptr->timeval.NowMicros ();
            printf ("%lu \n", target_ptr->interval_);
        */
        target_ptr->bufnode_->Unlock ();
        goto retry;  // continue to insert the key which didn't insert successfully.
#endif
    }
}
...
DIR_RETRY:
    ...
    if (D_RO (target)->local_depth == D_RO (dir)->depth) {
        ...
        {
            std::lock_guard<std::mutex> _BCF (bufferConfig._BCF_mtx);
            bufferConfig._bufferContainFlag->clear ();
            bufferConfig._bufferNotContainFlag->clear ();
            ...
            for (int i = 0; i < D_RO (_dir)->capacity;) {
                ...
                if (D_RW (D_RW (D_RW (_dir)->segment)[i])->buf_flag) {
                    bufferConfig._bufferContainFlag->insert (i);
                } else {
                    bufferConfig._bufferNotContainFlag->insert (i);
                }
            ...
            bufferConfig.bufferContainFlag = bufferConfig._bufferContainFlag;
            bufferConfig.bufferNotContainFlag = bufferConfig._bufferNotContainFlag;
        }
        ...
        x = (f_hash >> (8 * sizeof (f_hash) - D_RO (dir)->depth));
        std::lock_guard<std::mutex> BCF (bufferConfig.set_mutex);
        if (D_RO (dir)->depth == D_RO (target)->local_depth + 1) {
            if (x % 2 == 0) {
                if (D_RW (D_RW (D_RW (dir)->segment)[x + 1])->buf_flag) {
                    bufferConfig.bufferContainFlag->erase (x + 1);
                } else {
                    bufferConfig.bufferNotContainFlag->erase (x + 1);
                }
                D_RW (D_RW (dir)->segment)[x + 1] = s[1];
                if (D_RW (D_RW (D_RW (dir)->segment)[x + 1])->buf_flag) {
                    bufferConfig.bufferContainFlag->insert (x + 1);
                } else {
                    bufferConfig.bufferNotContainFlag->insert (x + 1);
                }
#ifdef INPLACE
                ...
                if (D_RW (D_RW (D_RW (dir)->segment)[x])->buf_flag) {
                    bufferConfig.bufferContainFlag->erase (x);
                } else {
                    bufferConfig.bufferNotContainFlag->erase (x);
                }
                D_RW (D_RW (dir)->segment)[x] = s[0];
                if (D_RW (D_RW (D_RW (dir)->segment)[x])->buf_flag) {
                    bufferConfig.bufferContainFlag->insert (x);
                } else {
                    bufferConfig.bufferNotContainFlag->insert (x);
                }
                pmemobj_persist (pop, (char*)&D_RO (D_RO (dir)->segment)[x],
            ...
            for (int i = 0; i < stride / 2; ++i) {
                bufferConfig.bufferContainFlag->erase (loc + stride / 2 + i);
                if (oflag && D_RW (s[1])->buf_flag) {
                    bufferConfig.bufferContainFlag->insert (loc + stride / 2);
                    oflag = false;
                }
                if (iflag && !D_RW (s[1])->buf_flag) {
                    bufferConfig.bufferNotContainFlag->insert (loc + stride / 2);
                    iflag = false;
                }
            ...
            oflag = true;
            for (int i = 0; i < stride / 2; ++i) {
                bufferConfig.bufferContainFlag->erase (loc + i);
                if (oflag && D_RW (s[0])->buf_flag) {
                    bufferConfig.bufferContainFlag->insert (loc);
                    oflag = false;
                }
                if (iflag && !D_RW (s[0])->buf_flag) {
                    bufferConfig.bufferNotContainFlag->insert (loc);
                    iflag = false;
                }
...
void CCEH::mergeBufAndSplitWhenNeeded (PMEMobjpool* pop, WriteBuffer* bufnode, Segment_toid& target,
    ...
    MERGE_SPLIT_RETRY:
        if (D_RO (target)->local_depth == D_RO (dir)->depth) {  // double the directory
            ...
            printf ("Double the directory BF \n");
            ...
            auto x = (f_hash >> (8 * sizeof (f_hash) - D_RO (dir)->depth));
            // Only one dir entry will point to this segment
            std::lock_guard<std::mutex> BCF (bufferConfig.set_mutex);
            if (D_RO (dir)->depth == D_RO (target)->local_depth + 1) {
                if (x % 2 == 0) {
                    if (D_RW (D_RW (D_RW (dir)->segment)[x + 1])->buf_flag) {
                ...
                for (int i = 0; i < stride / 2; ++i) {
                    bufferConfig.bufferContainFlag->erase (loc + stride / 2 + i);
                    if (oflag && D_RW (split_segment)->buf_flag) {
                        bufferConfig.bufferContainFlag->insert (loc + stride / 2);
                        oflag = false;
                    }
                    if (iflag && !D_RW (new_segment)->buf_flag) {
                        bufferConfig.bufferNotContainFlag->insert (loc + stride / 2);
                        iflag = false;
                    }
                ...
                for (int i = 0; i < stride / 2; ++i) {
                    bufferConfig.bufferContainFlag->erase (loc + i);
                    if (oflag && D_RW (new_segment)->buf_flag) {
                        bufferConfig.bufferContainFlag->insert (loc);
                        oflag = false;
                    }
                    if (iflag && !D_RW (new_segment)->buf_flag) {
        ...
        // printf ("all records in bufnode has been merge to new_segment_dram \n");
...
size_t CCEH::Capacity (void) {
    ...
    printf ("segment/dir_entries = %2.2f\n", cnt * 1.0 / D_RO (dir)->capacity);
